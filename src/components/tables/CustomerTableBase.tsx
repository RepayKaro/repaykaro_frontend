...
interface CustomerTableBaseProps {
    endpoint: string;
    title: string;
    type?: 'current-customers' | 'total-customers';
}
...
const CustomerTableBase: React.FC<CustomerTableBaseProps> = ({ endpoint, title, type }) => {
    // State management
    ...
    const fetchCustomers = useCallback(async () => {
        ...
        try {
            ...
            const response = await fetch(
                `${endpoint}?${params.toString()}&type=${type}`,
                { credentials: 'include' }
            );
    ...
    }, [currentPage, pageSize, selectedStatus, appliedFilters, endpoint, type]);
    ...
    const handleDownload = async (type: 'all' | 'filtered', tabName?: CustomerTableBaseProps['type']) => {
        ...
        try {
            ...
            if (type === 'filtered') {
                ...
                params.set('type', tabName ? tabName : type);
            ...
            if (result.success && result.data.length > 0) {
                ...
                generateExcel(result.data, type === 'all' ? `all_${title.toLowerCase().replace(' ', '_')}` : `filtered_${title.toLowerCase().replace(' ', '_')}`);
    ...
    return (
        <div className="overflow-hidden  border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03] relative">
            {loading && (
            ...
            <div className="p-4 space-y-4 md:space-y-0 md:flex md:items-center md:justify-between md:gap-4 border-b border-gray-200 dark:border-white/[0.05]">
                ...
                <div className="flex items-center gap-2 justify-end">
                    ...
                    <div className="relative">
                        ...
                        >
                            <DropdownItem onClick={() => { handleDownload('filtered', type); setIsDropdownOpen(false); }}>
                                Download
                            </DropdownItem>
                        </Dropdown>
                    </div>
...
export default CustomerTableBase;